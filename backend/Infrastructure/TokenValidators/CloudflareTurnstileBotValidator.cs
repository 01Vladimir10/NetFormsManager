...
{
    private const string CloudflareTurnstileVerifyUrl = "https://challenges.cloudflare.com/turnstile/v0/siteverify";

    public async Task<bool> ValidateAsync(string token, Dictionary<string, string> parameters, HttpContext httpContext)
    {
        if (!parameters.TryGetValue(SecretParameterName, out var secret)) return false;
        ...
        try
        {
            ...
            var response = await httpClient.PostAsync(CloudflareTurnstileVerifyUrl, content);
            response.EnsureSuccessStatusCode();

            var result = await response.Content.ReadFromJsonAsync<CloudflareTurnstileResponse>(JsonSerializerOptions.Web);
    ...
    public static BotValidationProviderMetadata GetMetadata() => new(
        ...
        {
            [SecretParameterName] = BotValidationParameterInfo.Required("Turnstile secret"),
        },
        parameters =>
        {
    ...
    private class CloudflareTurnstileResponse
    {
